...
/**
 * Lead form on /mobility (Base-member campaign). Same `leads` table,
 * but source = "mobility" so these can be filtered in the portal, and
 * the note leads with the program name so it reads at a glance.
 */
export async function submitMobilityLead(
  formData: FormData
): Promise<LeadResult> {
  const first_name = String(formData.get("first_name") ?? "").trim();
  const last_name = String(formData.get("last_name") ?? "").trim();
  ...
  const fullNotes = ["Mobility with Pilates inquiry.", notes]
    .filter(Boolean)
    .join("\n\n");
    ...
    try {
      ...
      await resend.emails.send({
        ...
        text: [
          ...
          `Email: ${email}`,
          `Phone: ${phone}`,
          "Program: Mobility with Pilates (The Base)",
          "",
          "Message:",
          notes || "(no message)",
        ].join("\n"),
